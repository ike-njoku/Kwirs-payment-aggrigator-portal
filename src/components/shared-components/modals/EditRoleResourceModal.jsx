...
const EditRoleResourceModal = ({ isOpen, onClose, roleResourceId, selectedRoleResource, selectedRole }) => {
  ...
  const handleUpdateRoleResource = async () => {
    ...
    try {
      ...
      if (StatusCode === 200) {
        toast.success(StatusMessage || "Role resource updated successfully!");
        onClose();
      } else {
        toast.error(StatusMessage || "Update failed.");
      }
    } catch (error) {
    ...
    <ModalLayout handleCloseModal={onClose}>
      <div className="w-full p-5">
        ...
        <form className="w-full mt-4" onSubmit={(e) => { e.preventDefault(); handleUpdateRoleResource(); }}>
          ...
          <input type="hidden" value={userEmail} readOnly className="w-full border-b-2 border-gray-300 h-[45px] rounded-md my-2 bg-gray-100 px-3 text-gray-700" />
